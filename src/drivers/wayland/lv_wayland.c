...
int lv_wayland_get_display_size(const char *name, int32_t *width, int32_t *height)
{
	...
	*height = 480;
	...
	{
		if (!strcmp(name, lv_wl_ctx.outputs[i].name))
		{
			*width = lv_wl_ctx.outputs[i].width;
			*height = lv_wl_ctx.outputs[i].height;
			return i;
		}
	}
	return 0;
...
void lv_wayland_init(void)
{
	...
	if (xdg_mgr)
	{
		...
		//wl_list_for_each(info, &lv_wl_ctx.outputs, link)
		for (int i = 0; i < lv_wl_ctx.wl_output_count; i++)
		{
			info = &lv_wl_ctx.outputs[i];
						...
						int32_t refresh)
{
	...
	printf("Pixel Resolution: width %d height %d\n", width, height);
}
						  ...
						  uint32_t version)
{
	...
	else if (strcmp(interface, wl_output_interface.name) == 0)
	{
		if (app->wl_output_count < LV_WAYLAND_MAX_OUTPUTS)
		{
			...
			app->outputs[app->wl_output_count].wl_name = name;
			app->wl_output_count++;
		}
	}
